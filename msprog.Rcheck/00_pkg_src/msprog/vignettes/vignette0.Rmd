...
```
    where: `event_sequence` specifies the order of the events and `CDW` counts CDW events.

**Note.** In this example, we called `MSprog()` without specifying the `event` argument - which defaults to `firstCDW` (only detect the first CDW event). For this reason, (1) in `output_edss$results`, `nevent` can only be 0 or 1; (2) in `output_edss$event_count`, `event_sequence` can contain a maximum of one event, CDW count can only be 0 or 1, and CDI column is omitted. In a multiple-event setting (`event=multiple`), more than one event per subject can be detected. See [the relevant section](#multiple).
