...
```
    where: `event_sequence` specifies the order of the events; the other columns count the events of each kind (CDW; relapse-associated worsening, [RAW](#raw); progression independent of relapse activity, [PIRA](#pira), see [@lublin2014; @kappos2018; @silent2019]; CDI column is omitted).[^seq]
...
[^seq]: In this example, we did not specify the argument `event` - which defaults to `firstCDW` (only detect the first CDW event). For this reason, (1) `event_sequence` can contain a maximum of one event; (2) `nevent` in `output_edss$results` can only be 0 or 1; (3) CDI column is omitted. In a multiple-event setting (`event=multiple`), more than one event per subject can be detected. See [the relevant section](#multiple).
...
```
The baseline has now been moved to the confirmation visit of the first CDI event (visit 3, EDSS=3.5). With respect to this baseline, a subsequent PIRA event is detected at visit 4 (EDSS=5), confirmed at visit 5.
...
* `include_dates`: if `TRUE`, the results will include the dates of: event onset; baseline; last visit before event onset at clinically meaningful score distance from it; confirmation visit.
* `include_value`: if `TRUE`, the results will include the outcome value at: event onset; baseline; last visit before event onset at clinically meaningful score distance from it; confirmation visit.
