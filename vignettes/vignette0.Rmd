...
The chosen confirmation period depends on the type of study and on the frequency of visits, and can be set in `MSprog()` by using the `conf_days` argument. A tolerance interval around `conf_days` can be specified using the `conf_tol_days` argument, given as a sequence of two integers (left and right tolerance)[^lrtol]. Further, the `conf_unbounded_right` argument allows event confirmation to occur at any visit *after* `conf_days` (i.e., right tolerance set to infinity). Let's see two examples.

i. A common setting for clinical trial data would be: `conf_days=7*12`, `conf_tol_days=0`, `conf_unbounded_right=T`, i.e., "confirmation over 12 or more weeks".

ii. A common setting for observational data would be: `conf_days=7*24`, `conf_tol_days=c(0,7*12)`, `conf_unbounded_right=F`, i.e., the confirmation visit must lie between 24 weeks after the event, and 36 weeks after the event.
...
* Suppose the maximum follow-up in the population is 96 weeks excluding the baseline visit. If one wants to detect progressions sustained over the whole follow-up, coding it as "progression confirmed over 96 weeks or more" (`conf_days=7*96`, `conf_tol_days=0`, `conf_unbounded_right=T`) would discard progressions of patients whose follow-up is shorter than 96 weeks. In a scenario where patients have different follow-up lengths, the `require_sust_days` argument offers a more reliable way of implementing this, e.g., as "progression confirmed over 12 weeks or more and sustained for the whole follow-up period" (`conf_days=7*12`, `conf_tol_days=0`, `conf_unbounded_right=T`, `require_sust_days=Inf`). 

* In observational data with coarsely- and unevenly-spaced visits, the `require_sust_days` argument may be used to detect sustained progression (e.g., `require_sust_days=7*96`) while still requiring the presence of a confirmation visit within a specified interval from the event (e.g., `conf_days=7*12`, `conf_tol_days=c(0,7*12)`). 
