...
print.MSprogOutput <- function(x, ...) {
  ...
  text <- paste0(
    ...
    ' days', ifelse(s$conf_tol_days[2]==Inf, ' or more', ''),
    ifelse(s$conf_tol_days[1]>0, paste0(' (with a tolerance of ', s$conf_tol_days[1],
                                        ifelse(s$conf_tol_days[1]==s$conf_tol_days[2], ' days on both sides)',
                                            paste0(' days on the left', ifelse(s$conf_tol_days[2] %in% c(0, Inf), ')',
                                                                        paste0(' and ', s$conf_tol_days[2], ' days on the right)'))))),
           ifelse(s$conf_tol_days[2] %in% c(0, Inf), '', paste0(' (with a tolerance of ',
    ...
    ifelse(s$baseline=='fixed',
           ...
           paste0('A roving baseline scheme was applied where the reference value was ',
                  ifelse(s$baseline=='roving_impr', 'updated after each confirmed improvement event. ',
                         ifelse(s$baseline=='roving_wors', 'updated after each confirmed worsening event. ',
